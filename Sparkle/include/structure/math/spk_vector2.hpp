...
IVector2<TType> operator+(UType scalar, const IVector2<TType>& vec)
{
    return IVector2<TType>(scalar + vec.x, scalar + vec.y);
}
...
{
    template<typename TType>
    struct hash<IVector2<TType>>
    {
        size_t operator()(const IVector2<TType>& vec) const
        {
