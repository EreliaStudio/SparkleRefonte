...
namespace spk
{
	...
	void Font::Atlas::_loadGlyph(const wchar_t& p_char)
	{
		...
		setData(_pixels, _size, format());
	}

	void Font::_loadFileData(const std::filesystem::path& p_path)
	{
		_fontData = FileUtils::readFileAsString(p_path);

		stbtt_InitFont(&_fontInfo, reinterpret_cast<const unsigned char*>(_fontData.c_str()), 0);
	}
}